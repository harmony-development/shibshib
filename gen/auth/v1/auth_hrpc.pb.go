...
func (DefaultAuthService) StreamSteps(context.Context, chan *StreamStepsRequest) (chan *StreamStepsResponse, error) {
	return nil, errors.New("unimplemented")
}
...
func (h *AuthServiceHandler) Routes() map[string]server.RawHandler {
	return map[string]server.RawHandler{
		...
		"/protocol.auth.v1.AuthService/StreamSteps": server.NewStreamingHandler(&StreamStepsRequest{}, func(c context.Context, req chan proto.Message) (chan proto.Message, error) {
			res, err := h.Server.StreamSteps(c, req.(chan *StreamStepsRequest))
